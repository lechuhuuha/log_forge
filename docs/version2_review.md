...
- **Backpressure instead of synchronous blocking** – the buffered `workCh` lets the server absorb short bursts faster than v1’s direct `SaveBatch`, which blocked every handler until the file lock released (`service/ingestion.go:38-55` vs. `repo/file_repo.go:41-82`).
...
4. **Persisting one record at a time regresses write amplification** – Consumers call `store.SaveBatch` with slices of length one (`service/consumer.go:89-114`), which forces `repo/file_repo.go:66-96` to open+close files for every message. V1 wrote the entire HTTP batch per file lock, so file descriptors and system calls were amortized. With high throughput, v2 may spend more time thrashing file locks than producing to Kafka.
