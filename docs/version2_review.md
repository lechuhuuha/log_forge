...
1. **Batches drop on producer errors without retry** – Producers log Kafka write failures and increment `ingest_errors_total` but ultimately drop the batch after configured retries. The client already received `202 Accepted`, so failures silently lose data unless monitored.
2. **Queue buffer can exhaust memory under sustained load** – The producer work queue can hold many batches; a burst of large requests can pin memory if Kafka is unavailable and nothing drains on exit.
3. **Persisting one record at a time regresses write amplification** – Consumers call repo `SaveBatch` with slices of length one, which forces `repo/file_repo.go` to open+close files for every message. V1 wrote the entire HTTP batch per file lock, so file descriptors and system calls were amortized. With high throughput, v2 may spend more time thrashing file locks than producing to Kafka.
4. **Channel workers never shut down** – Producer workers rely on channel close; ensure shutdown closes the channel or cancels workers to avoid leakage.
5. **API semantics diverge from v1 with no client signal** – `/logs` returns `202 Accepted` once enqueued, not persisted. Without surfacing offsets or a queue depth metric, clients cannot tell whether their data survives.
